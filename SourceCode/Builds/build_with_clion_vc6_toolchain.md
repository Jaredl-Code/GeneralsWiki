...
> This guide assumes the user will be using the default install
...
1. To configure the CMake profiles, do the following:
...
2. Edit the selected configuration, and do the following: (Has to be repeated for both `z_generals` and `g_generals`)
   ...
   - Set **Executable** to the path of your game install plus the built executable name, for example:
     ...
     This ensures that the game will correctly start after clicking the run button in CLion.
...
1. Click **Run** on the toolbar, or build the project and run the executable from the
   game directory.
